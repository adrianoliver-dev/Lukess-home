...
export function ProductBadges({ isNew, isNewUntil, discount, discountExpiresAt, isBestSeller, collection }: ProductBadgesProps): React.ReactElement {
  ...
  return (
    <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
      {isActuallyNew && (
        <span className="bg-lukess-gold text-gray-900 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md">
          Nuevo
        </span>
      )}

      {hasActiveDiscount && (
        <span className="bg-discount text-white text-[10px] font-semibold px-2.5 py-1 rounded-md flex items-center gap-1">
      ...
      {isBestSeller && (
        <span className="bg-gray-900 text-white text-[10px] font-bold px-2.5 py-1 rounded-md flex items-center gap-1">
